...
Notes: a node's first query may pay a one-time CUDA probe (~seconds, cached afterwards). Config/node list: `../.data/nodes_monitor/config.toml` if present, else the bundled template. Management actions (occupy/kill/release) live in `manager.sh`, not here.
